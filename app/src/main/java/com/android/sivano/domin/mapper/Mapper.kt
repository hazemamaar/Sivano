...
fun AddOrRemoveCartDto.toAddCartResponse():AddOrRemoveCartModel=AddOrRemoveCartModel( id,ProductCart(product.id,product.image, price = product.price),quantity)
...
fun GetAllFavoriteProductsDto.toGetAllFavorite():GetAllFavorites=GetAllFavorites(favoriteData = favoriteData.map { it.toFavoriteDate() })
fun FavoriteDataDto.toFavoriteDate():FavoriteData= FavoriteData(id,
    FavoriteAndCartProductModel(product.description,product.discount,product.id,product.image,product.name,product.old_price,product.price))
fun AddOrRemoveFavoriteDto.toAddOrRemoveFavorite():AddOrRemoveFavorite=AddOrRemoveFavorite(id,FavoriteProduct(product.discount,product.id,product.image,product.old_price,product.price))
