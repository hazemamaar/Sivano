...
class AuthViewModel @Inject constructor(
    ...
    fun register(userInfoDto: com.android.sivano.data.entities.auth.UserInfoDto)=viewModelScope.launch {

        useCaseRegister(userInfoDto).onEach {
            registerMutableLiveData.postValue(it)
        }.launchIn(viewModelScope)
    }
    fun login(userInfoDto: com.android.sivano.data.entities.auth.UserInfoDto){
        useCaseLogin(userInfoDto).onEach {
            loginMutableLiveData.postValue(it)
        }.launchIn(viewModelScope)

    }
    fun fcmToken(fcmToken: com.android.sivano.data.entities.auth.FcmTokenOtd){
